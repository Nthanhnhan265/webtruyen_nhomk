...
const StoryModal: React.FC<StoryModalProps> = ({ onClose, onSuccess }) => {
  ...
  const [coverFile, setCoverFile] = useState<File | Blob | undefined>()
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!storyName.trim() || storyName.length < 1 || storyName.length > 255) {
      newErrors.storyName =
        'Tên truyện không được để trống và phải từ 1-255 ký tự.'
    } else if (/[!@#$%^&*(),.?":{}|<>]/.test(storyName)) {
      newErrors.storyName =
        'Tên truyện chỉ bao gồm chữ, số và không chứa ký tự đặc biệt.'
    }
    ...
    //     return;
    // }
    if (coverFile instanceof File || coverFile === undefined) {
    ...
    if (coverFile instanceof File) {
      formData.append('cover', coverFile) // Chỉ gọi nếu coverFile là File
    }
          ...
          <div className=" lg:flex lg:grid-cols-2 gap-5 mb-4 justify-normal">
            <div className="w-full lg:w-80 me-5">
              ...
              <div>
                ...
                />
                {errors.storyName && (
                  <p className="text-red-500 text-sm">{errors.storyName}</p>
                )}
              </div>
              {/* {từ khóa} */}
            ...
            <div className="w-full px-2">
              ...
              <div className="max-h-32 overflow-y-auto border border-gray-300 rounded p-2">
                <div className="flex flex-col">
                  {genres
                    .slice(0, showMore ? genres.length : 1)
                    .map((genre: Genre) => (
                      <div
                        key={genre.id}
