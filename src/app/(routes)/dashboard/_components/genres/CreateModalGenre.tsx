...
interface CreateModalGenreProps {
  ...
  onSuccess: (data?: Genre) => void // Function to call on successful genre creation
}

const CreateModalGenre: React.FC<CreateModalGenreProps> = ({
  show,
  onClose,
  onSuccess,
}) => {
  ...
  const validateForm = () => {
    ...
    if (!genre_name) {
      ...
      genre_name.length < 1 ||
      genre_name.length > 255 ||
      /[^a-zA-Z0-9\sàáạảãâầấậẩẫèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữđ]/.test(
        genre_name,
      )
    ) {
      formErrors.genre_name =
        'Vui lòng nhập tên thể loại hợp lệ (1 - 255 ký tự và không chứa ký tự đặc biệt).'
      isValid = false
    }
    ...
    if (
      ...
      formErrors.description =
        'Vui lòng nhập mô tả hợp lệ (không vượt quá 500 ký tự).'
      isValid = false
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    onSuccess() // Refresh the genre list
          ...
          <div className="mb-4">
            ...
            <input
              ...
              className={`w-full p-2 border ${
                errors.genre_name ? 'border-red-500' : 'border-gray-300'
              } rounded`}
              placeholder="Nhập tên thể loại"
            />
            {errors.genre_name && (
              <p className="text-red-500">{errors.genre_name}</p>
            )}
          </div>
          <div className="mb-4">
