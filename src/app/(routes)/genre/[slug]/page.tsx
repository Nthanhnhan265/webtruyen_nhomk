...
const GenrePage = () => {
  ...
  const fetchBooks = async () => {
    try {
      ...
      const res = await fetch(`http://localhost:3000/api/story/genre/${slug}?page=${page}`);
    ...
    <div>
      ...
      <div>
        ...
        <div className="ml-4 sm:ml-14 mt-8 mb-4">
          <span className="font-bold">Truyện thể loại <span className="font-bold">{genreName}</span></span>
        </div>
              ...
              {books.map((book, index) => (
                <div key={index} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-4 mb-4">
                  ...
                  <div className="col-span-1 sm:col-span-9">
                    ...
                    <br />
                    <span>Thể loại:</span> <Link href={`/genre/${slug}`}>{genreName}</Link>
                    <br />
